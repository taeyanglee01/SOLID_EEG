...
# TODO : ddp should be implemented in other file not in this py file
...
def get_parser_args(parser):
    ...
    parser.add_argument('--total_steps', type=int, default=100_000, help='learning total steps')
...
#################### DDP specific functions ####################

def setup_distributed():
    """
    Initialize distributed training environment.
    Returns rank, world_size, and local_rank.
    Falls back to single-GPU mode if environment variables are not set.
    """
    ...
    if world_size > 1:
        ...
        print(f"[Rank {rank}] Initialized DDP with world_size={world_size}, local_rank={local_rank}")
...
def collate_fn_filter_none(batch):
    ...
    This handles cases where dataset.__getitem__ returns None for corrupted/missing data.
    """
    ...
    if len(batch) == 0:
        # If entire batch is None, return None (will be handled by DataLoader)
        return None

    # Use default collate on filtered batch
    return default_collate(batch)

def prepare_dataloader_ddp(data_loader_dict, rank, world_size, batch_size, num_workers=4):
    """
    Wrap existing DataLoaders with DistributedSampler for DDP training.
    Preserves all original DataLoader characteristics including custom collate_fn.

    Args:
        data_loader_dict: Dictionary with keys 'train', 'val', 'test' containing DataLoader objects
        rank: Current process rank
        world_size: Total number of processes
        batch_size: Batch size per GPU (can be None to use original)
        num_workers: Number of workers for DataLoader (can be None to use original)

    Returns:
        Dictionary of DataLoaders with DistributedSampler, and train_sampler for epoch setting
    """
    if world_size == 1:
        # Single-GPU mode: return original loaders
        return data_loader_dict, None

    new_loaders = {}
    train_sampler = None

    for split_name, loader in data_loader_dict.items():
        dataset = loader.dataset

        # Extract original DataLoader settings
        original_batch_size = loader.batch_size
        original_num_workers = loader.num_workers
        original_pin_memory = loader.pin_memory
        original_drop_last = loader.drop_last
        original_timeout = loader.timeout
        original_collate_fn = loader.collate_fn
        original_persistent_workers = getattr(loader, 'persistent_workers', False)
        original_prefetch_factor = getattr(loader, 'prefetch_factor', None)

        # Use provided batch_size/num_workers if specified, else use original
        batch_size_to_use = batch_size if batch_size is not None else original_batch_size
        num_workers_to_use = num_workers if num_workers is not None else original_num_workers
        ...
        sampler = DistributedSampler(
            ...
            rank=rank,
            shuffle=shuffle,
            drop_last=False  # Let DistributedSampler handle drop_last
        )

        # Store train sampler for epoch setting in training loop
        if split_name == 'train':
            train_sampler = sampler

        # Wrap original collate_fn with None-filtering if needed
        if original_collate_fn is not None:
            # Wrap the original collate_fn to filter None samples
            def wrapped_collate_fn(batch, original_fn=original_collate_fn):
                # Filter out None samples
                batch = [item for item in batch if item is not None]
                if len(batch) == 0:
                    return None
            ...
            collate_fn_to_use = wrapped_collate_fn
        else:
            # No custom collate_fn, use our filter
            collate_fn_to_use = collate_fn_filter_none

        # Create new DataLoader preserving all original characteristics
        new_loader_kwargs = {
            'dataset': dataset,
            ...
            'pin_memory': original_pin_memory,
            'drop_last': original_drop_last,
            'timeout': original_timeout,
        }

        # Add optional kwargs if they exist
        if original_persistent_workers and num_workers_to_use > 0:
            new_loader_kwargs['persistent_workers'] = original_persistent_workers
        if original_prefetch_factor is not None and num_workers_to_use > 0:
            new_loader_kwargs['prefetch_factor'] = original_prefetch_factor

        new_loader = DataLoader(**new_loader_kwargs)

        new_loaders[split_name] = new_loader

    return new_loaders, train_sampler
...
test_loader = DataLoader(test_grid_dataset, batch_size=params.batch_size, shuffle=False, drop_last=False, num_workers=2, pin_memory=True)
