...
class EEGToGridCtx9_1sec(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        T = x0.shape[0]
        cond_thw = cond.unsqueeze(1).expand(-1, T, -1, -1) # (C,T,H,W)
        ...
        return x0.unsqueeze(0), loss_mask_hw.unsqueeze(0).unsqueeze(0), cond_thw, self.mean, self.std
...
class UNet(nn.Module):
    def __init__(self, base_dim=128, dim_mults=(1, 2, 4),
                 in_channels=2, vol_size=(200, H, W), dropout=0.0, groups=32,
                 down_stride=(1, 2, 2), up_scale=(1, 2, 2),
                 out_channels=1):
        super().__init__()
        self.vol_t, self.vol_h, self.vol_w = vol_size
        self.time_dim = base_dim * 4

        self.time_mlp = TimeMLP(base_dim)
        self.init = nn.Conv3d(in_channels, base_dim, 3, padding=1)
        ...
        for li, m in enumerate(dim_mults):
            out_ch = base_dim * m
            rb1 = ResnetBlock(in_ch, out_ch, self.time_dim, dropout, groups)
            self.downs.append(rb1); in_ch = out_ch; skip_channels.append(in_ch)

            rb2 = ResnetBlock(in_ch, out_ch, self.time_dim, dropout, groups)
            self.downs.append(rb2); in_ch = out_ch; skip_channels.append(in_ch)

            if li != len(dim_mults) - 1:
                # downsample (keep T, downsample H/W by default)
                self.downs.append(nn.Conv3d(in_ch, in_ch, 3, stride=down_stride, padding=1))
        ...
        for li, m in enumerate(reversed(dim_mults)):
            ...
            for _ in range(2):
                skip_ch = sc.pop()
                self.ups.append(ResnetBlock(in_ch + skip_ch, out_ch, self.time_dim, dropout, groups))
                self.kinds.append('res')
                in_ch = out_ch

            if li != len(dim_mults) - 1:
                self.ups.append(nn.Upsample(scale_factor=up_scale, mode='nearest'))
                self.kinds.append('up')
                self.ups.append(nn.Conv3d(in_ch, in_ch, 3, padding=1))
    ...
    def forward(self, x_cat, t):
        ...
        for kind, layer in zip(self.kinds, self.ups):
            if kind == 'res':
                s = skips.pop()
                # align (T,H,W) if needed
                if s.shape[-3:] != h.shape[-3:]:
                    s = F.interpolate(s, size=h.shape[-3:], mode='nearest')
                h = layer(torch.cat([h, s], dim=1), t_emb)
            else:
                h = layer(h)

        # final align to configured volume size (T,H,W)
        if h.shape[-3:] != (self.vol_t, self.vol_h, self.vol_w):
            h = F.interpolate(h, size=(self.vol_t, self.vol_h, self.vol_w), mode='nearest')
...
class GaussianDiffusion(nn.Module):
    def __init__(self, unet, image_size=(200,H,W), time_steps=params.time_steps, loss_type='l2'):
        super().__init__()
        self.unet = unet
    ...
    def _bcast(self, x):
        # (B,) -> (B,1,1,1,1) for (B,C,T,H,W)
        return x.view(-1, 1, 1, 1, 1)

    def q_sample(self, x0, t, noise):
        return self._bcast(self.sqrt_alpha_bar[t]) * x0 + \
               self._bcast(self.sqrt_one_minus_alpha_bar[t]) * noise
    ...
    def p_sample(self, xt, cond, t, clip=True):
        ...
        def bcast(x): return x.view(-1,1,1,1,1)
        if clip:
    ...
    def sample(self, cond, clip=False):  # clip=False often gives crisper fields
        b = cond.size(0)
        x = torch.randn((b,1,200,self.H,self.W), device=cond.device)
...
IN_CHANNELS = 4   # target(noised) + lat/lon + 9 past grids + 9 past masks = 21
unet = UNet(base_dim=128, dim_mults=(1,2,4), in_channels=IN_CHANNELS, vol_size=(200,H,W)).to(DEVICE)
...
def overlay_panel(test_loader, model, save_path=os.path.join(params.result_dir, 'ctx9_overlay.png'),
    ...
    with torch.inference_mode():
        ...
        if params.squash_tanh:
            raw_hat = inv_tanh_to_raw(xhat, mub[:,None,None,None,None], stdb[:,None,None,None,None]).squeeze(1)
            raw_gt  = inv_tanh_to_raw(xb,   mub[:,None,None,None,None], stdb[:,None,None,None,None]).squeeze(1)
        else:
        ...
        mb      = mb.squeeze(1).squeeze(1)
        if mb.ndim == 4:            # (B,T,H,W)
            mb = mb[:, :, t0]          # (B,H,W)
