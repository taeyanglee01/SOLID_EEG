...
class Unet(nn.Module):
    def __init__(self, dim, image_size, dim_multiply=(1, 2, 4, 8), channel=3, num_res_blocks=2,
        ...
        self.init_conv = nn.Conv2d(channel * 2 + 1, self.dim, kernel_size=(3, 3), padding=1)
    ...
    def forward(self, x, time, sparse_input=None, mask=None):
        """
...
#             B = x.shape[0]
#             x = torch.cat([x, sparse_input[:B], mask[:B, :1, ...]], dim=1)
        # Downward
        concat = list()
