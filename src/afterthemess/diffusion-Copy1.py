...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def sample(self, batch_size=128, return_all_timestep=False, clip=True, min1to1=False,
           sparse_input=None, mask=None):
        """
        ...
        for t in tqdm(reversed(range(0, self.time_step)), desc='DDPM Sampling', total=self.time_step, leave=False):
            x_t_minus_1 = self.p_sample(xt, t, clip, sparse_input=sparse_input, mask=mask)
