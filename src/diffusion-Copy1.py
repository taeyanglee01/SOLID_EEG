...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, batch_size=16, return_all_timestep=False, clip=True, min1to1=False,
        ...
        # noise = torch.randn_like(perceiver_input)
        # alpha = 0.95  # or experiment with 0.9, 0.98, etc.
        # xT = alpha * noise + (1 - alpha) * perceiver_input.detach()
    ...
    def red_guided_sample(
        self,
        perceiver_input,    # (B, C, H, W) - Coarse estimate from Perceiver
        sparse_input,       # (B, C, H, W) - Observed values
        mask,               # (B, 1, H, W) - Binary: 1 for observed, 0 for unobserved
        T=1000,
        lr=1e-3,
        sigma_x0=0.01
    ):
        """
        RED-Diff-style gradient-based sampling from sparse input + coarse guidance.
        Optimizes mu directly, where mu is the clean image.
        """
        B, C, H, W = perceiver_input.shape
        device = perceiver_input.device

        # === Step 1: Initialize mu from perceiver and make it optimizable ===
        # mu = perceiver_input.clone().detach().requires_grad_(True)
        # optimizer = torch.optim.Adam([mu], lr=lr)
        mu = perceiver_input.clone().detach()

        for step, t in enumerate(reversed(range(T))):
            mu = mu.detach().clone().requires_grad_(True)
            optimizer = torch.optim.Adam([mu], lr=lr)

            # === Recompute noise and time-dependent stuff ===
            noise_xt = torch.randn_like(mu)
            noise_mu = torch.randn_like(mu)

            alpha_t = self.alpha_bar[t]
            sqrt_alpha_t = alpha_t.sqrt()
            sqrt_1m_alpha_t = (1 - alpha_t).sqrt()

            x0_pred = mu + sigma_x0 * noise_mu
            x_t = sqrt_alpha_t * x0_pred + sqrt_1m_alpha_t * noise_xt

            model_input = torch.cat([x_t, sparse_input, mask], dim=1)

            with torch.no_grad():  # ✅ prevent graph retention
                pred_eps = self.unet(model_input, torch.full((B,), t, device=device, dtype=torch.long))

            # === Observation loss: guide outside hint ===
            obs_mask = (1.0 - mask)
            obs_loss = ((mu * obs_mask - perceiver_input * obs_mask) ** 2).mean()

            # === Denoising loss ===
            denoise_loss = ((pred_eps - noise_xt).detach() * mu).mean()

            # === Backprop ===
            total_loss = obs_loss + denoise_loss
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()


            if step % (T // 10) == 0 or t == 0:
                print(f"Step {step} | t={t} | Obs Loss: {obs_loss.item():.4f} | Denoise Loss: {denoise_loss.item():.4f}")
...
class DDIM_Sampler(nn.Module):
    ...
    def ddim_p_sample(self, model, xt, i, clip=True, sparse_input=None, mask=None):
        ...
        if sparse_input is not None and mask is not None:
            ...
            model_input = xt
        pred_noise = model.unet(model_input, batched_time)
