...
class Trainer:
    ...
    def train(self):
        ...
        def make_split_sparse(img_tensor, percent_total=0.2):
            """
            Randomly sample percent_total of pixels and split into 50% conditioning and 50% for loss.
            """
            ...
            # Sample total GT locations
            idx_total = torch.randperm(total_pixels, device=img_tensor.device)[:num_total]
            idx_cond = idx_total[:num_half]
            idx_supervise = idx_total[num_half:]

            cond_mask = torch.zeros(H * W, device=img_tensor.device)
            cond_mask[idx_cond] = 1
            cond_mask = cond_mask.view(1, 1, H, W).repeat(B, C, 1, 1)

            target_mask = torch.zeros(H * W, device=img_tensor.device)
            target_mask[idx_supervise] = 1
            target_mask = target_mask.view(1, 1, H, W).repeat(B, C, 1, 1)

            sparse_cond = img_tensor * cond_mask

            return sparse_cond, cond_mask, target_mask
            ...
            if cur_step != 0 and (cur_step % self.save_and_sample_every) == 0:
                ...
                with torch.inference_mode():
                    ...
                    for i, j in zip([True, False], ['clip', 'no_clip']):
                        ...
                        viz = torch.cat([
                            (image[:self.num_samples] + 1) / 2,              # GT
                            (sparse_vis + 1) / 2,                            # Sparse
                            mask_vis,                                        # Mask
                            (imgs + 1) / 2                                   # Generated
                        ], dim=0)
