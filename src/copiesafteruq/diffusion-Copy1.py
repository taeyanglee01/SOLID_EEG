...
def estimate_pixel_uncertainty(
    x_t: Tensor,
    t: int,
    model,                    # your U-Net: model(x, t) → εθ(x,t)
    alphas_cumprod: Tensor,   # diffusion.alpha_bar
    M: int = 30
) -> Tensor:
    alpha_bar_t = alphas_cumprod[t]
    sqrt_ab   = alpha_bar_t.sqrt()
    sqrt_1_ab = (1 - alpha_bar_t).sqrt()

    # 1) true score
    eps = model(x_t, t)

    # 2) reconstruct x0_hat
    x0_hat = (x_t - sqrt_1_ab * eps) / sqrt_ab

    # 3) Monte Carlo sampling
    scores = []
    for _ in range(M):
        noise   = torch.randn_like(x_t)
        x_hat_t = sqrt_ab * x0_hat + sqrt_1_ab * noise
        scores.append(model(x_hat_t, t))
    scores = torch.stack(scores, dim=0)

    # 4) pixel-wise variance
    return scores.var(dim=0, unbiased=False)
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample_with_uncertainty_guidance(
        self,
        batch_size,
        sparse_input,
        mask,
        ...
        """
        Sampling with uncertainty guidance (Algorithm 2).
        """
        device = self.device
        xT = torch.randn([batch_size, self.channel, self.image_size, self.image_size], device=device)
        xt = xT
        timesteps = list(reversed(range(self.time_step))) if end_step is None else list(reversed(range(start_step, end_step)))

        for t in tqdm(timesteps, desc='Uncertainty-Guided Sampling'):
            t_tensor = torch.full((batch_size,), t, device=device, dtype=torch.long)

            xt.requires_grad = False  # xt does not require grad

            # Step 1: predict noise ε
            eps = self.unet(xt, t_tensor, sparse_input=sparse_input, mask=mask)
            eps = eps.requires_grad_(True)

            # Step 2: x0_hat and Monte Carlo samples
            sqrt_ab = self.alpha_bar[t].sqrt()
            sqrt_1_ab = (1 - self.alpha_bar[t]).sqrt()
            x0_hat = (xt - sqrt_1_ab * eps) / sqrt_ab

            eps_samples = []
            for _ in range(M):
                noise = torch.randn_like(xt)
                x_hat_t = sqrt_ab * x0_hat + sqrt_1_ab * noise
                eps_sample = self.unet(x_hat_t, t_tensor, sparse_input=sparse_input, mask=mask)
                eps_samples.append(eps_sample)
            eps_stack = torch.stack(eps_samples, dim=0)
            uncertainty_map = (eps_stack - eps.unsqueeze(0)).pow(2).mean(dim=0).sum(dim=1, keepdim=True)  # shape [B, 1, H, W]

            # Step 3: get high-uncertainty mask
            threshold_val = torch.quantile(uncertainty_map.view(batch_size, -1), p / 100.0, dim=1, keepdim=True)
            mask_uncertain = (uncertainty_map.view(batch_size, -1) > threshold_val).view_as(uncertainty_map).float()

            # Step 4: compute gradient and update
            grad = torch.autograd.grad(outputs=(uncertainty_map * mask_uncertain).sum(), inputs=eps)[0]
            eps_updated = eps - lambda_ * grad

            # Step 5: denoise step using updated score
            x0 = self.sqrt_recip_alpha_bar[t] * xt - self.sqrt_recip_alpha_bar_min_1[t] * eps_updated
            if clip:
                x0.clamp_(-1., 1.)
            mean = self.mean_tilde_x0_coeff[t] * x0 + self.mean_tilde_xt_coeff[t] * xt
            var = self.beta_tilde[t]
    ...
    def forward(self, img, sparse_input=None, mask=None, loss_mask=None):
        ...
        predicted_noise = self.unet(noised_image, t, sparse_input=sparse_input, mask=mask)
    ...
    def p_sample(self, xt, t, clip=True, sparse_input=None, mask=None):
        ...
        # if sparse_input is not None and mask is not None:
        #     # Blend sparse input into xt based on the mask
        #     blended_input = xt * (1 - mask) + sparse_input * mask
        #     model_input = torch.cat([blended_input, mask], dim=1)
        # else:
        model_input = xt

        pred_noise = self.unet(model_input, batched_time, sparse_input=sparse_input, mask=mask)
    ...
    def sample(self, batch_size=16, return_all_timestep=False, clip=True, min1to1=False,
           sparse_input=None, mask=None,estimate_uq_per_step=True):
        """
        ...
        # Create dummy sparse_input and mask filled with zeros for now
        # Later, you'll pass real sparse inputs as optional args
        ...
        for t in tqdm(reversed(range(0, self.time_step)), desc='DDPM Sampling', total=self.time_step, leave=False):
            x_t_minus_1 = self.p_sample(xt, t, clip, sparse_input, mask)
            
            if estimate_uq_per_step:
                def score_fn(x_in: torch.Tensor, tt: int):
                    B = x_in.shape[0]
                    t_tensor = torch.full((B,), tt, device=x_in.device, dtype=torch.long)
                    # concatenate sparse inputs as before
                    return self.unet(x_in, t_tensor, sparse_input=sparse_input, mask=mask)



                U_t = estimate_pixel_uncertainty(
                    x_t=xt,
                    t=t,
                    model=score_fn,
                    alphas_cumprod=self.alpha_bar,
                    M=5
                )
        ...
        return images, U_list if estimate_uq_per_step else None
...
class DDIM_Sampler(nn.Module):
    ...
    @torch.inference_mode()
    def ddim_p_sample(self, model, xt, i, clip=True, sparse_input=None, mask=None):
        """
        ...
        model_input = xt
        pred_noise = model.unet(model_input, batched_time, sparse_input=sparse_input, mask=mask)
    ...
    @torch.inference_mode()
    def sample(self, diffusion_model, batch_size, noise=None, return_all_timestep=False, clip=True, min1to1=False, sparse_input=None, mask=None):
        """
        ...
        for i in tqdm(reversed(range(0, self.ddim_steps)), desc='DDIM Sampling', total=self.ddim_steps, leave=False):
            x_t_minus_1 = self.ddim_p_sample(diffusion_model, xt, i, clip, sparse_input=sparse_input, mask=mask)
            denoised_intermediates.append(x_t_minus_1)
            xt = x_t_minus_1
