...
class Trainer:
    ...
    def train(self):
        ...
        for cur_step in stepTQDM:
            ...
            sparse_input = image * cond_mask
        
            
#             if cur_step == 0:
#                 self.seen_one_sample = False
#                 self.tracked_hash = None
#                 self.tracked_step_first = None
#                 self.tracked_step_repeat = None

                
            
#             if not self.seen_one_sample:
#                 # Pick the first sample in the batch and start tracking it
#                 img_bytes = image[0].cpu().numpy().tobytes()
#                 self.tracked_hash = hash(img_bytes)
#                 self.tracked_step_first = cur_step
#                 self.seen_one_sample = True

#             if self.tracked_step_repeat is None:
#                 for i in range(B):
#                     img_bytes = image[i].cpu().numpy().tobytes()
#                     img_hash = hash(img_bytes)

#                     if img_hash == self.tracked_hash:
#                         os.makedirs('./debug_repeat', exist_ok=True)

#                         composite = torch.cat([
#                             (image[i:i+1] + 1) / 2,
#                             (sparse_input[i:i+1] + 1) / 2,
#                             cond_mask[i:i+1],
#                             target_mask[i:i+1]
#                         ], dim=0)
#                         grid = make_grid(composite, nrow=1)

#                         if cur_step == self.tracked_step_first:
#                             save_image(grid, f'./debug_repeat/sample_step{cur_step}_FIRST.png')
#                             print(f"[DEBUG] Saved first occurrence of tracked sample at step {cur_step}")
#                         else:
#                             save_image(grid, f'./debug_repeat/sample_step{cur_step}_REPEAT.png')
#                             self.tracked_step_repeat = cur_step
#                             print(f"[DEBUG] Saved repeat occurrence at step {cur_step}")
#                         break  # stop after first match
            ...
            loss = self.diffusion_model(image, sparse_input=sparse_input, mask=cond_mask, loss_mask=target_mask)

        
           # loss = self.diffusion_model(image, sparse_input=sparse_input, mask=mask)
            loss.backward()
            ...
            if cur_step != 0 and (cur_step % self.save_and_sample_every) == 0:
                ...
                with torch.inference_mode():
                    ...
                    for i, j in zip([True, False], ['clip', 'no_clip']):
                        ...
                        # Resize sparse and mask to match generated
                        ...
                        cond_vis = cond_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)
                        target_vis = target_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)
                        # Compose side-by-side grid for each sample
                        viz = torch.cat([
                            (image[:8] + 1) / 2,
                            (sparse_input[:8] + 1) / 2,
                            cond_vis,
