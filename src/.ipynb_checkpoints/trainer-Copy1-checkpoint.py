...
class Trainer:
    def __init__(self, diffusion_model, dataset, batch_size=32, lr=2e-5, total_step=100000, ddim_samplers=None,
                 ...
                 max_grad_norm=1., tensorboard=False, exp_name=None, clip=True, sparsity_mode='random_epoch', sparsity_pattern='random',sparsity_level=0.2, block_size=5,num_blocks = 5, **dataset_kwargs):
        """
       ...
       dataSet = dataset_wrapper(dataset, self.image_size, **dataset_kwargs)
        ...
        self.perceiver = CascadedPerceiverIO(
            input_dim=3 + 192,  # 3 RGB channels + 96*2 Fourier Features
            queries_dim=192,
            logits_dim=3,
        ...
        self.perceiver.load_state_dict(torch.load("./NS_5percent.pth"))
        self.perceiver.eval()
        for p in self.perceiver.parameters():
            p.requires_grad = False

        # Fourier encoder
        self.fourier = GaussianFourierFeatures(in_features=2, mapping_size=96, scale=15.0).to(self.device)
        self.fourier.load_state_dict(torch.load("./fourierNS_5percent.pth"))
        self.fourier.eval()
        for p in self.fourier.parameters():
            p.requires_grad = False
        print("Loaded Perceiver and Fourier encoder weights.")

        # Coordinate grid (used as input to Fourier encoder)
        self.coords = create_coordinate_grid(64, 64, device=self.device)  # Shape: [H*W, 2] or similar
    ...
    def train(self):
        ...
        #     notification = make_notification('Tensorboard', color='blue')
        #     print(notification)
        #     print(colored('Tensorboard Available!', 'blue'))
        #     print(colored('Tensorboard name: {}'.format(self.tensorboard_name), 'blue'))
        #     print(colored('Launch Tensorboard by running following command on terminal', 'blue'))
        ...
        for cur_step in stepTQDM:
            ...
            # sparse_input, cond_mask, target_mask = make_split_sparse(image, percent_total=0.2)
            ...
            if isinstance(batch, (tuple, list)) and len(batch) == 2:
                image, sample_id = batch                  # image: (B,3,H,W), sample_id: (B,)
                image = image.to(self.device, non_blocking=True)
                # ensure Python ints for the sparsity controller keying
                sample_ids = [int(s.item()) for s in sample_id]
            else:
                image = batch.to(self.device, non_blocking=True)
                B = image.shape[0]
                # fallback: deterministic but slower; prefer dataset-provided sample_id
                sample_ids = [
                    int(hash(image[i].detach().cpu().numpy().tobytes()) & ((1 << 63) - 1))
                    for i in range(B)
                ]
            B, C, H, W = image.shape
            # sample_ids = [hash(image[i].cpu().numpy().tobytes()) for i in range(B)]
            ...
            # Default values for recon and alpha
            recon = torch.zeros_like(image)
            
            
            with torch.no_grad():
                recon = run_perceiver_reconstruction(
                    sparse_img=sparse_input.to(self.device),
                    mask=cond_mask,
                    model=self.perceiver,
                    fourier_encoder=self.fourier,
                    coords=self.coords
                ).to(self.device)
                recon = recon.clamp(-1, 1)
            
            
            if cur_step < 3:  # first few steps only
                ...
                recon_vis = (recon[:8] + 1) / 2
                cond_vis = cond_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)
                target_vis = target_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)
            ...
            loss = self.diffusion_model(image, sparse_input=sparse_input, perceiver_input=recon, mask=cond_mask, loss_mask=target_mask)
            
            # perceiver_loss = F.mse_loss(model_output * target_mask, recon * target_mask)
                ...
                with torch.inference_mode():
                    ...
                    for i, j in zip([True, False], ['clip', 'no_clip']):
                        ...
                        for n in batches:
                            ...
                            # Run per-image Perceiver reconstruction
                            recon_list = []
                            for i in range(n):
                                recon_i = run_perceiver_reconstruction(
                                    sparse_img=sparse_batch[i].unsqueeze(0),  # shape [1, 3, H, W]
                                    mask=mask_batch[i].unsqueeze(0),
                                    model=self.perceiver,
                                    fourier_encoder=self.fourier,
                                    coords=self.coords
                                ).clamp(-1, 1)
                            ...
                            recon_batch = torch.cat(recon_list, dim=0)  # [n, 3, H, W]
                        

                            # Fuse
                            # fused_batch = mask_batch * sparse_batch + (1 - mask_batch) * recon_batch

                            # Sample using per-image fused input
                            sampled = self.diffusion_model.sample(
                                batch_size=n,
                                sparse_input=sparse_batch,
                                perceiver_input=recon_batch,
                                mask=mask_batch,
                                clip=i
                            )
                        ...
                        sparse_vis = sparse_input[:self.num_samples]
                        mask_vis = cond_mask[:self.num_samples]
                        perceiver_vis = recon_batch[:8]  # add this

                        cond_vis = cond_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)
                        target_vis = target_mask[:8][:, :1, :, :].repeat(1, 3, 1, 1)

                        # Normalize all image tensors to [0,1] if needed
                        viz = torch.cat([
                            (image[:8] + 1) / 2,
                            (sparse_input[:8] + 1) / 2,
                            (perceiver_vis + 1) / 2,  # add Perceiver recon
                            imgs[:8]  ,
                            cond_vis,
                            target_vis,
                        ], dim=0)
                        ...
                        sparsity_str = "10"
                        save_image(grid, os.path.join(self.ddpm_result_folder, j, f'composite_{cur_step}_sparsity{sparsity_str}.png'))



                        if self.writer is not None:
                            self.writer.add_image('Composite (GT | Sparse | Mask | Recon)', grid, cur_step)
                ...
                if cur_step != 0 and (cur_step % sampler.sample_every) == 0:
                            ...
                            for i, j in zip([True, False], ['clip', 'no_clip']):
                                ...
                                if sampler.fixed_noise:
                                    ...
                                    imgs = list(map(lambda n: self.diffusion_model.sample(
                                        ...
                                        perceiver_input=recon_batch[:n],  # add this
                                        mask=cond_mask[:n],
                                        clip=i
                                    ), batches))
                                imgs = torch.cat(imgs, dim=0)
                    ...
                    if sampler.calculate_fid:
                        sample_ = lambda batch_size, clip=True, min1to1=False: sampler.sample(
                                ...
                                perceiver_input=torch.zeros(batch_size, self.diffusion_model.channel, self.image_size, self.image_size, device=self.device),  # ← ADD THIS
