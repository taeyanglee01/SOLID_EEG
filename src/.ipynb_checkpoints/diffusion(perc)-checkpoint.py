...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img, sparse_input=None,perceiver_input = None, mask=None,loss_mask=None):
        """
        ...
        if sparse_input is not None and mask is not None:
            model_input = torch.cat([noised_image, sparse_input, perceiver_input, mask], dim=1)
        else:
    ...
    def p_sample(self, xt, t, clip=True, sparse_input=None, perceiver_input = None, mask=None):
        ...
        if sparse_input is not None and mask is not None:
            ...
            model_input = torch.cat([xt, sparse_input, perceiver_input, mask], dim=1)  # shape (B, 9, H, W)
        else:
    ...
    def sample(self, batch_size=16, return_all_timestep=False, clip=True, min1to1=False,
           sparse_input=None, perceiver_input = None, mask=None, use_p = False):
        """
        ...
        # fused = mask * sparse_input + (1 - mask) * perceiver_output
        # perceiver_out = (1 - mask) * sparse_input  # Extract perceiver prediction only
        # xT = perceiver_out + 0.1 * torch.randn_like(perceiver_out)
        
        noise = torch.randn_like(perceiver_input)
        alpha = 0.97  # small influence from Perceiver
        ...
        for t in tqdm(reversed(range(0, self.time_step)), desc='DDPM Sampling', total=self.time_step, leave=False):
            # xt = mask * sparse_input + (1 - mask) * xt
            # sparse_hints = mask * sparse_input
...
class DDIM_Sampler(nn.Module):
    ...
    def ddim_p_sample(self, model, xt, i, clip=True, sparse_input=None, perceiver_input = None, mask=None):
        ...
        batched_time = torch.full((xt.shape[0],), t, device=self.device, dtype=torch.long)
        if sparse_input is not None and mask is not None:
            model_input = torch.cat([xt, sparse_input, perceiver_input, mask], dim=1)
        else:
            model_input = xt
