...
class Unet(nn.Module):
    def __init__(self, dim, image_size, dim_multiply=(1, 2, 4, 8), channel=3, num_res_blocks=2,
        ...
        self.init_conv = nn.Conv2d(channel * 3, self.dim, kernel_size=(3, 3), padding=1)
        ...
        for level in range(self.num_resolutions):
            ...
            for block in range(num_res_blocks):
                ...
                if self.resolution[level] in attn_resolutions:
                    self.down_path.append(ResnetAttentionBlock(d_in_, d_out, self.time_emb_dim,self.cond_dim, dropout, groups))
                else:
                    self.down_path.append(ResnetBlock(d_in_, d_out, self.time_emb_dim,self.cond_dim, dropout, groups))
        ...
        self.middle_resnet_attention = ResnetAttentionBlock(mid_dim, mid_dim, self.time_emb_dim, self.cond_dim, dropout, groups)
        self.middle_resnet = ResnetBlock(mid_dim, mid_dim, self.time_emb_dim,self.cond_dim, dropout, groups)
        ...
        for level in reversed(range(self.num_resolutions)):
            ...
            for block in range(num_res_blocks + 1):
                ...
                if self.resolution[level] in attn_resolutions:
                    self.up_path.append(ResnetAttentionBlock(d_in, d_out, self.time_emb_dim,self.cond_dim, dropout, groups))
                else:
                    self.up_path.append(ResnetBlock(d_in, d_out, self.time_emb_dim,self.cond_dim, dropout, groups))
            if level != 0:
    ...
    def forward(self, x, time, x_coarse=None, sparse_input=None, mask=None):
        ...
        if x_coarse is not None:
            cond = x_coarse.mean(dim=[2, 3])           # (B, C)
            cond = self.condition_proj(cond)           # (B, cond_dim)
        else:
