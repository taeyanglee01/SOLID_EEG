...
class Trainer:
    def __init__(self, diffusion_model, dataset, batch_size=32, lr=2e-5, total_step=100000, ddim_samplers=None,
                 ...
                 max_grad_norm=1., tensorboard=False, exp_name=None, clip=True, sparsity_mode='random_epoch', sparsity_pattern='random',sparsity_level=0.2,                   block_size=5,num_blocks = 5):
        """
        ...
        self.perceiver.load_state_dict(torch.load("./perceiver_cifar10_sparse10.pth"))
        self.perceiver.eval()
        ...
        self.fourier.load_state_dict(torch.load("./fourier_encoder.pth"))
        self.fourier.eval()
        ...
        self.coords = create_coordinate_grid(32, 32, device=self.device)  # Shape: [H*W, 2] or similar
    ...
    def train(self):
        ...
        for cur_step in stepTQDM:
            ...
            loss = self.diffusion_model(image, sparse_input=sparse_input, perceiver_input=recon_input, mask=cond_mask, loss_mask=target_mask)
                    ...
                    for i, j in zip([True, False], ['clip', 'no_clip']):
                        ...
                        for n in batches:
                            ...
                            sampled = self.diffusion_model.sample(
                                ...
                                perceiver_input=perceiver_input,
                                mask=mask_batch,
                                clip=i
                            )
